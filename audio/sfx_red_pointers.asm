; Gen 1 Kanto on Crystal: parallel SFX table, structurally mirroring
; RedMusic (audio/music_red_pointers.asm). Selected by the MUSIC_SOURCE
; option bit via GetSFXHeader (audio/engine.asm). Swaps in the 17
; confidently-name-matched Red SFX plus 17 dedicated appended ids (#45/#47,
; #49/#51); every other entry keeps the original Crystal sound. Generated by
; tools/port_sfx.py.

RedSFX::
; entries correspond to SFX_* constants
	table_width 3
	dba Sfx_DexFanfare5079
	dba Sfx_Item
	dba Sfx_RedCaughtMon
	dba Sfx_PokeballsPlacedOnTable
	dba Sfx_Potion
	dba Sfx_FullHeal
	dba Sfx_Menu
	dba Sfx_ReadText
	dba Sfx_ReadText2
	dba Sfx_DexFanfare2049
	dba Sfx_DexFanfare80109
	dba Sfx_Poison
	dba Sfx_GotSafariBalls
	dba Sfx_BootPc
	dba Sfx_ShutDownPc
	dba Sfx_ChoosePcOption
	dba Sfx_EscapeRope
	dba Sfx_PushButton
	dba Sfx_SecondPartOfItemfinder
	dba Sfx_WarpTo
	dba Sfx_WarpFrom
	dba Sfx_ChangeDexMode
	dba Sfx_JumpOverLedge
	dba Sfx_GrassRustle
	dba Sfx_RedFly
	dba Sfx_Wrong
	dba Sfx_Squeak
	dba Sfx_Strength
	dba Sfx_Boat
	dba Sfx_WallOpen
	dba Sfx_PlacePuzzlePieceDown
	dba Sfx_EnterDoor
	dba Sfx_SwitchPokemon
	dba Sfx_Tally
	dba Sfx_Transaction
	dba Sfx_ExitBuilding
	dba Sfx_Bump
	dba Sfx_RedSave
	dba Sfx_RedPokeflute
	dba Sfx_ElevatorEnd
	dba Sfx_ThrowBall
	dba Sfx_RedBallPoof
	dba Sfx_Faint
	dba Sfx_RedRun
	dba Sfx_SlotMachineStart
	dba Sfx_Fanfare
	dba Sfx_RedPeck
	dba Sfx_Kinesis
	dba Sfx_Lick
	dba Sfx_RedPound
	dba Sfx_MovePuzzlePiece
	dba Sfx_CometPunch
	dba Sfx_MegaPunch
	dba Sfx_Scratch
	dba Sfx_Vicegrip
	dba Sfx_RazorWind
	dba Sfx_RedCut
	dba Sfx_WingAttack
	dba Sfx_Whirlwind
	dba Sfx_Bind
	dba Sfx_RedVineWhip
	dba Sfx_DoubleKick
	dba Sfx_MegaKick
	dba Sfx_Headbutt
	dba Sfx_HornAttack
	dba Sfx_Tackle
	dba Sfx_PoisonSting
	dba Sfx_Powder
	dba Sfx_RedDoubleslap
	dba Sfx_Bite
	dba Sfx_JumpKick
	dba Sfx_Stomp
	dba Sfx_TailWhip
	dba Sfx_KarateChop
	dba Sfx_Submission
	dba Sfx_WaterGun
	dba Sfx_SwordsDance
	dba Sfx_Thunder
	dba Sfx_Supersonic
	dba Sfx_Leer
	dba Sfx_Ember
	dba Sfx_Bubblebeam
	dba Sfx_HydroPump
	dba Sfx_Surf
	dba Sfx_RedPsybeam
	dba Sfx_Charge
	dba Sfx_Thundershock
	dba Sfx_Psychic
	dba Sfx_Screech
	dba Sfx_BoneClub
	dba Sfx_Sharpen
	dba Sfx_EggBomb
	dba Sfx_Sing
	dba Sfx_HyperBeam
	dba Sfx_Shine
	dba Sfx_Unknown5F
	dba Sfx_Unknown60
	dba Sfx_Unknown61
	dba Sfx_SwitchPockets
	dba Sfx_Unknown63
	dba Sfx_Burn
	dba Sfx_TitleScreenEntrance
	dba Sfx_Unknown66
	dba Sfx_GetCoinFromSlots
	dba Sfx_PayDay
	dba Sfx_Metronome
	dba Sfx_Call
	dba Sfx_HangUp
	dba Sfx_NoSignal
	dba Sfx_Sandstorm
	dba Sfx_Elevator
	dba Sfx_Protect
	dba Sfx_Sketch
	dba Sfx_RainDance
	dba Sfx_Aeroblast
	dba Sfx_Spark
	dba Sfx_Curse
	dba Sfx_Rage
	dba Sfx_Thief
	dba Sfx_Thief2
	dba Sfx_SpiderWeb
	dba Sfx_MindReader
	dba Sfx_Nightmare
	dba Sfx_Snore
	dba Sfx_SweetKiss
	dba Sfx_SweetKiss2
	dba Sfx_BellyDrum
	dba Sfx_Toxic
	dba Sfx_SludgeBomb
	dba Sfx_Foresight
	dba Sfx_Spite
	dba Sfx_Outrage
	dba Sfx_PerishSong
	dba Sfx_GigaDrain
	dba Sfx_Attract
	dba Sfx_Kinesis2
	dba Sfx_ZapCannon
	dba Sfx_MeanLook
	dba Sfx_HealBell
	dba Sfx_Return
	dba Sfx_ExpBar
	dba Sfx_MilkDrink
	dba Sfx_Present
	dba Sfx_MorningSun
	dba Sfx_RedLevelUp
	dba Sfx_KeyItem
	dba Sfx_Fanfare2
	dba Sfx_RegisterPhoneNumber
	dba Sfx_3rdPlace
	dba Sfx_GetEgg
	dba Sfx_GetEgg
	dba Sfx_MoveDeleted
	dba Sfx_2ndPlace
	dba Sfx_1stPlace
	dba Sfx_ChooseACard
	dba Sfx_GetTm
	dba Sfx_GetBadge
	dba Sfx_QuitSlots
	dba Sfx_EggCrack
	dba Sfx_DexFanfareLessThan20
	dba Sfx_DexFanfare140169
	dba Sfx_DexFanfare170199
	dba Sfx_DexFanfare200229
	dba Sfx_DexFanfare230Plus
	dba Sfx_Evolved
	dba Sfx_MasterBall
	dba Sfx_EggHatch
	dba Sfx_GsIntroCharizardFireball
	dba Sfx_GsIntroPokemonAppears
	dba Sfx_Flash
	dba Sfx_GameFreakLogoGs
	dba Sfx_RedNotVeryEffective
	dba Sfx_RedDamage
	dba Sfx_RedSuperEffective
	dba Sfx_BallBounce
	dba Sfx_Moonlight
	dba Sfx_Encore
	dba Sfx_BeatUp
	dba Sfx_BatonPass
	dba Sfx_BallWobble
	dba Sfx_SweetScent
	dba Sfx_SweetScent2
	dba Sfx_HitEndOfExpBar
	dba Sfx_GiveTrademon
	dba Sfx_GetTrademon
	dba Sfx_TrainArrived
	dba Sfx_StopSlot
	dba Sfx_2Boops
	dba Sfx_GlassTing
	dba Sfx_GlassTing2
	dba Sfx_IntroUnown1
	dba Sfx_IntroUnown2
	dba Sfx_IntroUnown3
	dba Sfx_DittoPopUp
	dba Sfx_DittoTransform
	dba Sfx_IntroSuicune1
	dba Sfx_IntroPichu
	dba Sfx_IntroSuicune2
	dba Sfx_IntroSuicune3
	dba Sfx_DittoBounce
	dba Sfx_IntroSuicune4
	dba Sfx_GameFreakPresents
	dba Sfx_Tingle
	dba Sfx_RedIntroWhoosh
	dba Sfx_TwoPcBeeps
	dba Sfx_4NoteDitty
	dba Sfx_Twinkle
	dba Sfx_RedHornDrill
	dba Sfx_RedPsychicM
	dba Sfx_RedTrainerAppeared
	dba Sfx_RedBattle09
	dba Sfx_RedBattle0D
	dba Sfx_RedBattle0F
	dba Sfx_RedBattle14
	dba Sfx_RedBattle19
	dba Sfx_RedBattle1B
	dba Sfx_RedBattle1E
	dba Sfx_RedBattle1E
	dba Sfx_RedBattle21
	dba Sfx_RedBattle24
	dba Sfx_RedBattle25
	dba Sfx_RedBattle29
	dba Sfx_RedBattle2C
	dba Sfx_RedBattle2F
	assert_table_length NUM_SFX

GetRedSFXHeader::
; Reached via farcall from GetSFXHeader (audio/engine.asm), already banked into
; RedSFX's own bank -- so, unlike that caller's stock-table path, this is a plain
; in-bank read, no far-read needed.
; in: de = sfx id. out: wMusicBank set, de = sfx header address. Clobbers a, hl.
	ld hl, RedSFX
	add hl, de
	add hl, de
	add hl, de
	ld a, [hli]
	ld [wMusicBank], a
	ld e, [hl]
	inc hl
	ld d, [hl]
	ret
