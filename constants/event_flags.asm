...
; Kanto hidden items
	...
	const EVENT_ROUTE_25_HIDDEN_POTION ; unused: replaced by the real Gen 1 Route 25 hidden items below
...
; Vermilion Gym trash can switch puzzle (Gen 1 port)
	const EVENT_VERMILIONGYM_1ST_LOCK_OPENED
	const EVENT_VERMILIONGYM_2ND_LOCK_OPENED
; Celadon Gym Gen 1 TM (Erika gives SOLARBEAM, not GIGA DRAIN, to match Gen 1)
	const EVENT_GOT_TM22_SOLARBEAM
; Fuchsia Gym Gen 1 port (Koga, not Janine, at his original first-encounter levels)
...
; Saffron Gym Gen 1 port (Sabrina's own Gen 1 roster; TM_PSYCHIC_M substitutes for
; Gen 1's TM46 PSYWAVE, which has no Crystal TM slot)
	...
	const EVENT_GOT_TM_PSYCHIC_SAFFRON_GYM
...
; Kanto Gym Leaders
	...
	const EVENT_BEAT_VIRIDIAN_GYM_GIOVANNI ; Gen 1: Viridian Gym's true (secret) leader, not Blue
; PokefanM
...
; Kanto people
	...
	const EVENT_VIRIDIAN_GYM_BLUE ; unused: Blue is not the Viridian Gym leader (see EVENT_BEAT_VIRIDIAN_GYM_GIOVANNI)
...
; Kanto itemballs
	const EVENT_PICKED_UP_FOCUS_BAND ; unused: Fighting Dojo no longer has a Focus Band (Gen 1 fidelity)
...
; Gen 1 Safari Zone field item balls
	...
	const EVENT_SAFARI_ZONE_EAST_TM_EGG_BOMB ; Gen1 TM39; Gen2 gives TM_SWIFT
	const EVENT_SAFARI_ZONE_NORTH_PROTEIN
	const EVENT_SAFARI_ZONE_NORTH_TM_SKULL_BASH ; Gen1 TM40; Gen2 gives TM_MUD_SLAP
...
; Cinnabar Gym trainer wall (NEEDS-MAP port; Blaine himself stays at Seafoam, see CinnabarIsland sign)
...
; Viridian Forest (NEEDS-MAP port)
...
; Route 1: Mart-employee Youngster's one-time Potion sample
...
; Rocket Hideout B2F/B3F Gen1 field itemballs
...
; Silph Co 2F-6F (batch SI1)
	...
	const EVENT_BEAT_SILPH_CO_ROCKETSROCK ; set by SilphCo11F on beating Giovanni (== Gen 1 EVENT_BEAT_SILPH_CO_GIOVANNI)
...
; Silph Co 7F-11F (batch SI2)
	...
	const EVENT_BEAT_SILPH_CO_11F_ROCKET_1
	const EVENT_BEAT_SILPH_CO_11F_ROCKET_2
	const EVENT_GOT_MASTER_BALL_FROM_SILPH_PRESIDENT
; Viridian Gym (Giovanni's true identity as gym leader, 8-trainer invisible-wall maze)
	const EVENT_BEAT_VIRIDIAN_GYM_COOLTRAINERM_1
	const EVENT_BEAT_VIRIDIAN_GYM_BLACKBELT_1
	const EVENT_BEAT_VIRIDIAN_GYM_GUITARIST_1
	const EVENT_BEAT_VIRIDIAN_GYM_BLACKBELT_2
	const EVENT_BEAT_VIRIDIAN_GYM_COOLTRAINERM_2
	const EVENT_BEAT_VIRIDIAN_GYM_BLACKBELT_3
	const EVENT_BEAT_VIRIDIAN_GYM_GUITARIST_2
	const EVENT_BEAT_VIRIDIAN_GYM_COOLTRAINERM_3
	const EVENT_PICKED_UP_VIRIDIAN_GYM_REVIVE
	const EVENT_GOT_TM_EARTHQUAKE_FROM_VIRIDIAN_GYM
; Fighting Dojo (Karate Master + HITMONLEE/HITMONCHAN choice gift)
	const EVENT_BEAT_KARATE_MASTER
	const EVENT_DEFEATED_FIGHTING_DOJO
	const EVENT_GOT_HITMONLEE
	const EVENT_GOT_HITMONCHAN
	const EVENT_BEAT_FIGHTINGDOJO_BLACKBELT_1
...
; Route 3 (3 Bug Catchers, 2 Youngsters, 3 Lasses)
...
; Route 8 (3 Super Nerd, 2 Gambler, 4 Lass)
...
; Route 9 (2 Jr.Trainer F, 2 Jr.Trainer M, 3 Hiker, 2 Bug Catcher)
...
; Route 21 (4 Fisher, 4 Swimmer subbed to SWIMMERM, 1 Cue Ball)
...
; Route 24 (Nugget Bridge: disguised Rocket leader + 2 Jr.Trainer M, 2 Lass, 1 Youngster, 1 Bug Catcher)
	const EVENT_BEAT_ROUTE_24_ROCKET_LEADER
	const EVENT_GOT_ROUTE_24_NUGGET
...
; Route 25 (3 Youngster, 1 Jr.Trainer M (reuses JR_TRAINER_M_6 data, shared w/ Route 24 in Gen 1),
; 2 Lass subbing for Gen 1's sprite-only SPRITE_COOLTRAINER_F, 3 Hiker)
...
; Route 12 (5 Fishers, 1 Jr.Trainer M, 1 Guitarist (Gen 1 Rocker), Snorlax roadblock)
...
; Route 13 (3 Bird Keepers, 4 Jr.Trainer F, 2 Beauty, 1 Biker)
...
; Route 15 (4 Jr.Trainer F, 2 Bird Keeper, 2 Beauty, 2 Biker)
...
; Route 18 (3 Bird Keeper)
	const EVENT_BEAT_ROUTE_18_TRAINER_0
	const EVENT_BEAT_ROUTE_18_TRAINER_1
	const EVENT_BEAT_ROUTE_18_TRAINER_2
; Route 19 (2 CoolTrainerM/SwimmerM, 5 SwimmerM, 3 Beauty -- added alongside the
; existing Dawn/Harold/Jerome/Tucker + rock-clearing Fisher quest, not replacing it)
...
; Rock Tunnel 1F (3 Hiker, 1 Super Nerd subbed to POKEMANIAC battle class, 3 CoolTrainerF
; subbed to JR_TRAINER_F battle class)
...
; Rock Tunnel B1F (2 new Hiker + reused CURTIS, 3 Super Nerd subbed to POKEMANIAC, 2 CoolTrainerF
; subbed to JR_TRAINER_F)
...
; S.S. Anne trainer rematch flags (never added when the map's trainers were first ported)
	const EVENT_FOUGHT_BLUE_SSANNE
	const EVENT_BEAT_SAILOR_SSANNE_1
	const EVENT_BEAT_SAILOR_SSANNE_2
	const EVENT_BEAT_SAILOR_SSANNE_3
	const EVENT_BEAT_SAILOR_SSANNE_4
	const EVENT_BEAT_SAILOR_SSANNE_5
	const EVENT_BEAT_FISHER_SSANNE
	const EVENT_BEAT_FISHER_SSANNEB1
	const EVENT_BEAT_LASS_SSANNE
	const EVENT_BEAT_LASS_SSANNE2
	const EVENT_BEAT_YOUNGSTER_SSANNE
	const EVENT_BEAT_GENTLEMAN_SSANNE_1
	const EVENT_BEAT_GENTLEMAN_SSANNE_2
	const EVENT_BEAT_GENTLEMAN_SSANNE_3
	const EVENT_BEAT_GENTLEMAN_SSANNE2_1
